...
public class GameView extends GridLayout {
		...
		setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					if(nowDown - lastDown < 200 && Math.abs(offsetX) < 5 && Math.abs(offsetY) < 5 && MainActivity.getMainActivity().reduceBom()){
		...
		for(int y=0; y<4; y++)
			for(int x=0; x<4; x++)
			{
				if(cardsMap[x][y].getNum() == 3){
					...
					MainActivity.getMainActivity().startBom();
					break ALL;
				}
			}
	}
	private void swipeLeft(){
		...
		MainActivity.getMainActivity().startMove();
//		if (merge) {
			checkBom();
			checkComplete();
			addRandomNum();
